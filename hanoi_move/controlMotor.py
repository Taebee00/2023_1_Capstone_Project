...
class ControlMotor:
    ...
    def setDefault(self):
        diff = []
        for i in range(3):
            diff.append(np.linspace(
                self.current_angles[i], self.default_angles[i], 50))
    ...
    def setPWMwithAngle(self, thetas, num):
        ...
        self.pwm.set_pwm(12, 0, AngleToRadian(
            90 - thetas[1] - 7 + thetas[2] - 21 + 20))
    ...
    def moveArmSlow(self):
        diff = []
        diff.append(np.linspace(
            self.current_angles[0], self.target_angles[0], 50))
        ...
        if self.current_angles[0] - self.target_angles[0] != 0:
            ...
            diff.append(np.linspace(
                self.current_angles[1], self.target_angles[1], 50))
            diff.append(np.linspace(
                self.current_angles[2], self.target_angles[2], 50))
            for i in range(50):
                for j in range(1, 3):
                    self.current_angles[j] = diff[j][i]
                    self.setPWMwithAngle(self.current_angles, j)
